...
import com.matrix.mq.utils.HashUtil;
...
public class LocalTransactionCheckerImpl implements LocalTransactionChecker {
    ...
    public TransactionStatus check(Message msg) {
        if (transactionCheck == null) {
            log.warn(
                    "mq Check 被回调但 LocalTransactionCheckerImpl 需要的 TransactionChecker尚未设置,原因:TransactionMessageTemplate的init方法尚未被调用");
        ...
        try {
            ...
            if (object == null) {
                log.error("SerializationUtils.deserialize 反序列化失败,自动回滚事务,message body:"
                        + (msg.getBody() == null ? null : new String(msg.getBody())));
                return TransactionStatus.RollbackTransaction;
            }
        } catch (Exception e) {
            log.error(
